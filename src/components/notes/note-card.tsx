...
export function NoteCard({
  ...
  return (
    <article className="notes-panel hud-frame notes-cosmos vn-stars relative overflow-hidden rounded-[28px] px-5 py-5 text-ink transition hover:-translate-y-1">
      <div className="notes-grid absolute inset-0 opacity-24" />
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(255,255,255,0.24),transparent_24%),linear-gradient(180deg,transparent,rgba(236,241,255,0.26)_76%,rgba(220,228,246,0.42)_100%)]" />
      <div className="relative grid gap-5 md:grid-cols-[auto_1fr_auto] md:items-start">
        <div className={`h-20 w-20 rounded-[24px] bg-gradient-to-br ${category?.accent ?? "from-white/30 to-white/10"} p-[1px]`}>
          <div className="flex h-full w-full items-end rounded-[23px] bg-white/52 px-3 py-2">
        ...
        <div>
          <p className="text-xs font-semibold tracking-[0.18em] text-ink-soft uppercase">
            save slot / {category?.signal ?? note.category}
          </p>
        ...
        <Link
          ...
          className="vn-ribbon inline-flex h-11 w-11 items-center justify-center rounded-full text-ink transition hover:-translate-y-0.5"
